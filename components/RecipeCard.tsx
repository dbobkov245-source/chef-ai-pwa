...
const RecipeCard: React.FC<RecipeCardProps> = ({ recipe, isSavedView = false, onRemove, onUpdate }) => {
  ...
  return (
    <div className="bg-surface-light rounded-2xl shadow-lg border border-gray-100 overflow-hidden mt-6 text-text-main animate-fade-in-up group relative">
      ...
      <div className="p-6 pb-4 pr-16 relative">
        {isEditing ? (
          <input 
            ...
            className="w-full text-2xl font-bold p-2 border border-primary rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
          />
        ) : (
        ...
        {isEditing ? (
          <textarea
            ...
            className="w-full mt-2 text-sm p-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-primary"
            rows={2}
          />
        ) : (
      ...
      <div className="flex gap-4 px-6 pb-6 overflow-x-auto scrollbar-hide">
        <div className="flex items-center gap-1.5 bg-background-light px-3 py-1.5 rounded-lg border border-gray-100 shrink-0">
